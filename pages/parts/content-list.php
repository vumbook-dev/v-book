    ...
    foreach($contentlist as $key => $value){
        if($value->chapter == $chapter){
            if(strlen($value->content) > 0){
                $viewbtn = '<span class="vb-view-content"><i class="fa fa-eye text-primary" aria-hidden="true"></i></span>';
            }else{ $viewbtn = ""; }
            ...
            $html .= '<li class="list-item-vbcontent d-flex justify-content-between align-items-center"><span>'.$value->cpart.'</span><span class="vb-btn-wrap">'.$viewbtn.'<span class="vb-dlt-content text-light" data-chapter="'.$chapter.'" data-key="'.$key.'"><i class="fa fa-trash-o text-danger" aria-hidden="true"></i></span><span class="showing-lightbox" data-chapter="'.$chapter.'" data-key="'.$key.'" data-name="'.$file.'">'.$button.'</span></span></li>';
        }
    }
