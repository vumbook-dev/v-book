...
if(isset($_POST['chapter']) && isset($_POST['title']) && isset($_POST['id'])){
    ...
    foreach($contentlist as $key => $value){
        if($value->chapter == $chapter){
            ...
            $html .= '<li class="list-item-vbcontent d-flex justify-content-between align-items-center"><span class="vb-cnt-title">'.$value->cpart.'</span><span class="vb-btn-wrap"><span class="vb-dlt-content text-light" data-chapter="'.$chapter.'" data-key="'.$key.'"><i class="fa fa-trash-o text-danger" aria-hidden="true"></i></span><span class="vb-edit-style showing-lightbox" data-chapter="'.$chapter.'" data-key="'.$key.'" data-name="'.$file.'">'.$button.'</span></span></li>';
        }
    }
